...
async function processActiveFile(document) {
  if (document && language(document)) {
    ...
    logger.log("start importCost---" + document.getText())
    logger.log("start importCost -- " + fileName)
    ...
    emitters[fileName].on('start', packages => {
      return flushDecorations(fileName, packages)
    });
    emitters[fileName].on('calculated', packageInfo => {
      logger.log(packageInfo);
      return calculated(packageInfo)
    });
